...
function initialize() {
    ...
    BTStat = document.getElementById("BTStat");
...
function clearMap() {
    ...
    ctx.clearRect(0, 0, 640, 360);
    //コンソール初期化
    canvasStat.innerHTML = '画像選択待ち';
    BTStat.innerHTML = '未受信';
    commandStat.innerHTML = '未送信';
    controllerStat.innerHTML = '初期化';
    droidroneStat.innerHTML = 'GPS未受信';
    serverStat.innerHTML = '初期化';
    imgArray = [];
    imgNo = 0;
...
function dirInit() {
	...
	if (dReq.readyState === 4) {
		...
		dirList = dReq.responseText.split(',');
                ...
                controllerStat.innerHTML = 'データ開始' + d.toLocaleString();
		...
		for ( var i = 0;  i < dirList.length; i++ ) {
                ...
                serverStat.innerHTML = 'ディレクトリ数: ' + dirList.length;
                clearMap();
 	    } else {
...
function canvasDraw(fileTime, no, str) {
    ...
    canvasStat.innerHTML = '撮影時間:' + time.toLocaleString()
...
function rDraw() {
    ...
    jReq.onreadystatechange = function () {
		...
		var infoImgArray = [];
		    ...
		    if ( jData.btr != null ) { BTStat.innerHTML = jData.btr; }
		    ...
		    rAccCircle.setMap(null);
                    ...
                    //現在位置
                    droidroneStat.innerHTML = "No.: " + jData.no + ", 現在位置　lat:" + jData.lat + ", lag:" + jData.lng;
                }
